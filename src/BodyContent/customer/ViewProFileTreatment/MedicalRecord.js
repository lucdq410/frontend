...
function ViewTreatmentProfile() {
  ...
  useEffect(() => {
    const fetchTreatmentProfile = async () => {
      try {
        const response = await axios.get(`https://qqqq-v7b6.onrender.com/api/treatment_profile/details?id=${id}`); // Replace with your API endpoint
        const profileData = response.data.treatmentProfile;
        setTreatmentProfile(profileData);

        // Fetch doctor information
        const doctorResponse = await axios.get(`https://qqqq-v7b6.onrender.com/api/account/doctor/details?id=${profileData.doctorID}`);
        const doctorData = doctorResponse.data.doctor;
        setDoctor(doctorData);

        // Fetch customer information
        const customerResponse = await axios.get(`https://qqqq-v7b6.onrender.com/api/account/customer/details?id=${profileData.customerID}`);
        const customerData = customerResponse.data.customer;
        setCustomer(customerData);

        // Fetch treatment_ins based on TreatmentProfile.id
        const treatmentInsResponse = await axios.get(`https://qqqq-v7b6.onrender.com/api/treatmentin/getAllByTreatmentProfile?id=${profileData.id}`);
